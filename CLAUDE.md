...
`03b` reuses `outputs/clustering_full.rds` if `03` has already produced it, so run `03`
first. `01` and `02` write only report inputs — `03`/`03b`/`04` do not read `data/`, they
each call `load_gii_data()` themselves.
