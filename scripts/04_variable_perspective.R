...
# sub-pillar using the LOCAL r_m_sub proxy (best_k_within_group /
# best_allocation_within_group, reused unchanged from the country perspective),
# then re-scored with the TRUE r_m against the full 78-indicator PCA - the
# local proxy is a tractable stand-in for a combinatorially infeasible full
# search; the true r_m is what's actually reported and compared.
#
...
print(paste("Loaded data with", nrow(indicator_data), "countries and", n_indicators, "indicators"))

print("Performing PCA on full dataset...")
...
print(paste("Components needed for", CONFIG$pca$variance_threshold * 100, "% variance:", pca_info$n_components))
...
print(paste("Strategy 2:", length(strategy2_vars), "variables, r_m =", round(strategy2_rm, 4)))
...
print(paste("Reflective indicators saved to:", best_per_subpillar_file))
...
print(paste("Strategy 1:", length(strategy1_vars), "variables, r_m =", round(strategy1_rm, 4)))
...
print(paste("Strategy 1a:", length(strategy1a_vars), "variables, r_m =", round(strategy1a_rm, 4)))
...
print(paste("Strategy 2a:", length(strategy2a_vars), "variables, r_m =", round(strategy2a_rm, 4)))
...
for (k in 1:4) {
  ...
  k_vars <- unlist(strsplit(k_results$variables, ","))
  k_rm <- r_m_named(k_vars)
...
print(paste("Flat-allocation sweep saved to:", dimension_file))
...
print(paste("Testing", length(k_values), "subset sizes x", n_draws, "random draws each"))

curve_results <- list()
for (k in k_values) {
  print(paste("k =", k, "(", round(k / n_indicators * 100), "% of indicators)"))
  ...
  print(paste("  Mean r_m:", round(mean(scores), 4)))
}
...
print(paste("Random-subset r_m curve saved to:", curve_file))
...
print(paste("r_m vs. #PCs comparison saved to:", pca_comparison_file))
...
print(paste("Selected variables saved to:", selected_vars_file))
...
print(paste("Optimal 21-variable subset saved to:", optimal_subset_file))
...
print(paste("Pillar allocation (budget=20) saved to:", allocation20_file))
